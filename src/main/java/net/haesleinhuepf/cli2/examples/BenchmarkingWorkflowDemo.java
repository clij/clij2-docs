...
public class BenchmarkingWorkflowDemo {
    ...
    private static void demoCLIJ() throws IOException {
        ...
        ClearCLBuffer flip = clij2.create(input);
        ClearCLBuffer flop = clij2.create(input);

        clij2.gaussianBlur(input, flop, (float) sigma, (float) sigma, (float) sigma);

        clij2.greaterConstant(flop, flip, 100.0f);

        clij2.erodeSphere(flip, flop);
        clij2.dilateSphere(flop, flip);

        clij2.multiplyImages(flop, input, flip);

        clij2.pull(flip).show();
