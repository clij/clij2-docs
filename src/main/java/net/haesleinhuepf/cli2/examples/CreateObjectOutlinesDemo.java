...
public class CreateObjectOutlinesDemo {
    public static void main(String[] args) {
        ...
        ClearCLBuffer extended = clij2.create(blobsGPU);
        ClearCLBuffer outline= clij2.create(blobsGPU);


        // apply threshold
        clij2.thresholdOtsu(blobsGPU, thresholded);

        clij2.dilateBox(thresholded, extended);

        clij2.binaryXOr(thresholded, extended, outline);



        // show result
        ImagePlus result = clij2.pullBinary(outline);
        result.show();
