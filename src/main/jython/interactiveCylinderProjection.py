...
while ((not gdp.wasCanceled()) and not (gdp.wasOKed())):
	...
	if (input is None):
		...
		cylinderProjectionTransformed_temp = clij2.create([cylinderProjection.getHeight(), cylinderProjection.getDepth(), cylinderProjection.getWidth()], input.getNativeType());
		cylinderProjectionTransformed = clij2.create([cylinderProjection.getDepth(), cylinderProjection.getHeight(), cylinderProjection.getWidth()], input.getNativeType());
		cylinderMaxProjection = clij2.create([cylinderProjectionTransformed.getWidth(), cylinderProjectionTransformed.getHeight()], input.getNativeType());
	...
	if (not stillValid):
		...
		clij2.resliceLeft(cylinderProjection, cylinderProjectionTransformed_temp);
		clij2.rotateRight(cylinderProjectionTransformed_temp, cylinderProjectionTransformed);
		clij2.maximumZProjection(cylinderProjectionTransformed, cylinderMaxProjection);
	...
	if (not stillValid):
		if (doSpotDetection):
			projectionImp = clij2.pull(cylinderMaxProjection);
			...
			if (circles is None):
				circles = clij2.create(cylinderMaxProjection);
			clij2.set(circles, 0);

	# Result visualisation
	if (not stillValid):
		impCylinderProjection = clij2.pull(cylinderMaxProjection);
